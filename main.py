...
class bt_dict_processor():
    ...
    def __init__(self):
        self.host_mac_list = {}
        self.command_counter_template = {"AIX": 0, "ALM": 0, "DAL": 0, "TSY": 0, "CDP": 0, "TST": 0, "ZIX": 0, "BRI": 0, "ALD": 0, "GRA": 0, "SCI": 0, "CPM": 0}
...
class Clock():
    ...
    def clock_sync(self, data):
        '''Central issues sync request and provides the current time. Ticks are calculated and display updated. Furthermore, time divergence between syncs are retained out of curiousity.'''
        if len(data) == 6 and isinstance(data, str):
            ...
            print(f'clock_sync:: data:{data}, ticks: {new_ticks}, was synced: {self.synced_state}, days since last sync event: {self.day_counter}, time divergence: {old_ticks-new_ticks}')
    ...
    def set_pwm(self, data):
        try:
            ...
            print(f"set_pwm:: could not convert value to int: '{pwm}'.")
        if (pwm >= 0) and (pwm <= 0xffff):
            self.display_pwm = pwm
            print(f'set_pwm:: PWM set to: {pwm}')
            self.display_current_time()
...
class LED_admin():
    ...
    def sparkel(self, on_off=1, freq=1, para=0):
        if on_off:
            print(f'sparkel:: enabled: {on_off}, frequency: {freq}')
            new_freq = 2
            freq_list_full = (1.64, 1.25, 1.117, 1, 0.8, 0.5, 0.33, 0.1, 0.08)
            freq_list = freq_list_full[:]

            if freq < 0.1:
                self.sparkel_level = 'low'
            elif freq > 2:
                self.sparkel_level = 'High'
            else:
                self.sparkel_level = 'Medium'
            ...
            try:
                self.sparkel_timer.deinit()
            except:
                print(f'sparkel_timer not setup.')
...
pixels.set_zone(zone_nr=3, rgb_str='ffffff')
utime.sleep_ms(10)
...
pixels.set_absolute(0, 47, '000000')
drawer_opened = 0
...
ALARM_DELETE_ID     = 'ALD'
SPARKEL_CONTROL_ID  = 'SCI'
CLOCK_PWM           = 'CPM' # PWM between 0x0000 and 0xFFFF
...
class BT_processor():
    ...
    def error_handler(self, error_type, data=''):
        ...
        debug.log({"error_type": error_type}, source="BT_processor", function="error_handler", level="ERROR")
    ...
    def MAC_whitelist_check(self):
        ...
        if self.MAC_address_hash in self.allowed_central_list:  # [:80]
            self.MAC_check = True
        debug.log({"MAC_check": self.MAC_check}, source="BT_processor", function="MAC_whitelist_check")
    ...
    def decrypt(self, cmd):
        if isinstance(cmd, bytes):
            ...
            debug.log({"cmd lenght": lenght, "cmd_bytes": cmd_bytes, "new": new}, source="BT_processor", function="decrypt")
            return new
        else:
            print(f"decrypt: Input not Bytes: '{cmd}'")
    ...
    def check_crc(self):
        ...
        debug.log({"d": d, "cmd": cmd, "crc_received": crc_received}, source="BT_processor", function="check_crc")
    ...
    def counter_integrity_check(self):
        ...
        debug.log({"command_counter": self.command_counter, "counter_rx": counter_rx}, source="BT_processor", function="counter_integrity_check")
    ...
    def parse(self):
        ...
        if SPARKEL_CONTROL_ID in command:
            on_off = d[1]
            freq = d[2]
            para = 0
            if len(d) >5:
                para = d[3]
            pixels.sparkel(on_off, freq, para)
            return 1

        if CLOCK_PWM in command:
            pwm = d[1]
            clock.set_pwm(pwm)
            return 1
    ...
    def process(self, MAC_address, payload):
        print(f"BT_process:: mac: {MAC_address}, payload: {payload}")
        ...
        if not self.MAC_check:
            ...
            return 0
        decrypted = self.decrypt(payload) # Decrypt, if MAC is ok.
        if decrypted == None:
            return 0
        else:
            self.payload_decrypted = decrypted # Decrypt, if MAC is ok.
...
while True:
    while uart.any() > 0:
        ...
        in_data = uart.read(1)
        rx_data += in_data
        #print(f"while loop:: data {in_data}")
        ...
        if connect_syntax in rx_data and not is_connected:
            ...
            is_connected = True
            is_MAC = True
            debug.log({"is_MAC": is_MAC, "is_data": is_data, "is_connected": is_connected}, source="bluetooth", function="main/while loop")
        ...
        if is_connected and is_MAC:
            if len(rx_data) == 12:
                ...
                debug.log({"is_MAC": is_MAC, "is_data": is_data, "is_connected": is_connected, "central_address": central_address}, source="bluetooth", function="main/while loop")
                rx_data = bytes()
        ...
        if is_connected and is_data:
            if disconnect_syntax in rx_data:
                ...
                debug.log({"is_MAC": is_MAC, "is_data": is_data, "is_connected": is_connected, "payload": payload}, source="bluetooth", function="main/while loop")
        ...
        if not is_connected and is_MAC:
            if len(rx_data) == 12:
                ...
                is_MAC = False
                debug.log({"is_MAC": is_MAC, "is_data": is_data, "is_connected": is_connected}, source="bluetooth", function="main/while loop", comment="Disconnected")
                print(f'Disconnected from address: {central_address}')
                rx_data = bytes()
                data_change = True
                debug.log({"data_change": data_change})
